...
class NationWantsToGuessApp(App):
    ...
    def create_past_rounds_section(self):
        ...
        section = BoxLayout(orientation='vertical', size_hint_y=0.12, padding=6, spacing=3)
        
        # Title
        title = Label(
            text='[b]ROUND SUMMARY[/b]',
            markup=True,
            color=ACCENT_COLOR,
            size_hint_y=0.2,
            font_size='10sp',
        ...
        summary_container = BoxLayout(orientation='vertical', size_hint_y=None, spacing=1)
        ...
        print(f"DEBUG: Current round={current_round}, Players={len(players)}, Total scores={len(scores)}")
        print(f"DEBUG: Players: {[(p['id'], p['name']) for p in players]}")
        ...
        try:
            ...
            for round_num in range(1, current_round):
                ...
                for player in players:
                    round_scores[player['id']] = {'name': player['name'], 'score': 0}
                
                round_score_count = 0
                for score in scores:
                    if score.get('round_number') == round_num:
                        pid = score.get('player_id')
                        pts = score.get('points', 0)
                        if pid in round_scores:
                            round_scores[pid]['score'] += pts
                            round_score_count += 1
                        print(f"  DEBUG: R{round_num} - Player {pid}: +{pts}")
                
                print(f"DEBUG: Round {round_num} found {round_score_count} scores")
                
                # Sort by score descending
                sorted_players = sorted(round_scores.items(), key=lambda x: x[1]['score'], reverse=True)
                
                # Build round text with all players
                score_parts = []
                medals = ['🥇', '🥈', '🥉']
                for idx, (pid, data) in enumerate(sorted_players):
                    medal = medals[idx] if idx < 3 else f"{idx+1}."
                    score_str = f"{data['score']:+.0f}" if data['score'] != 0 else "0"
                    score_parts.append(f"{medal}{data['name']}:{score_str}")
                
                round_label = Label(
                    text=f"R{round_num} {round_name}  →  {' | '.join(score_parts)}",
    ...
    def advance_to_next_round(self, instance):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.show_popup('Info', f'Round {self.current_game["current_round"]} Started!')
                # Refresh to same page - past rounds will show below
                self.show_scoring_screen()
            else:
                error_msg = response.text
